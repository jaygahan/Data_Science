...
set.seed(1738) # Set a random seed so that same sample can be 
                #reproduced in future runs

sample = sample.int(n = nrow(cleaned_housing), size = floor
                    (.8*nrow(cleaned_housing)), replace = F)
...
glm_cv_rmse # off by about $83000... it is a start
...
#rf_model = randomForest(median_house_value~. , data = train, ntree =500, imp
#ortance = TRUE)
rf_model = randomForest(train_x, y = train_y , ntree = 500, 
                        importance = TRUE)

names(rf_model) # these are all the diff things you can call from the model.

rf_model$importance

oob_prediction = predict(rf_model) #leaving out a data source forced OOB predictions

# you may have noticed that his is available using the $mse
...
housing <- arrange(.data = housing, desc(population))
housing
...
ggplot(
  data = housing,
  aes(x=population)) + geom_density() + 
  ggtitle("Distribution of homes by Population") +
  xlab("Population and Homes") +
  ylab("Density")
